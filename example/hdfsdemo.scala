...
/*object HDFSDemo2 {
  ...
  def main (args: Array[String]) = {
    val myTest = new HdfsDemo()
    myTest.createFile("peepoo", "poopoo")
  }
class HdfsDemo {
  //val username = "luke_bailey"
  ...
  def createFile(username: String, password: String): Unit = {
    ...
    if(isExisting) {
      println("Yes it does exist. Appending it...")
      val appender = fs.append(filepath)
      val newWriter = new PrintWriter(appender)
      newWriter.write(s"$username,$password\n")
      newWriter.close()
